...
class ProcessDBToText(ProcessBase):
    def __init__(self, context):
        ProcessBase.__init__(self,context)
        
    def build_db_to_text_task(self, task_name, dyn_params={}):
         ...
         op = PythonOperator(
            ...
            python_callable=self.db_to_text_op,
            op_kwargs=args
         )
         return op
    
    def db_to_text_op(self,task_name,dyn_params={}, **kwargs):
        ...
        if engine == 'glue':
            self.run_glue_job(task_name, task, 'DB_TO_TEXT')
        elif engine == 'emr':
            process_emr = ProcessEmr(self.context)
            emr_cluster_id=process_emr.create_cluster()
            process_emr.run_emr_job(task_name, task, 'DB_TO_TEXT', emr_cluster_id)
        elif engine == 'ray':
            process_ray = ProcessRay(self.context)
            process_ray.run_ray_job(task_name, task, 'DB_TO_TEXT')
        else:
            raise Exception(f"Unsupported engine: {engine}")
        return True


    def run_db_to_text_glue_job(self, task_name, task):
    ...
    def run_db_to_text_emr_job(self, task_name, task):
